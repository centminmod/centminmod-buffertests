...
' >> "${LOGDIR}/ss_output-rmem_max-${rmem_max}-wmem_max-${wmem_max}.log"
        ...
        for i in $(seq 1 $num_runs); do
            ...
            echo "wrk -t4 -c300 -d30s --breakout -s /root/tools/wrk-cmm/scripts/json.lua http://localhost/index.php"
            test_output=$(wrk -t4 -c300 -d30s --breakout -s /root/tools/wrk-cmm/scripts/json.lua http://localhost/index.php)
